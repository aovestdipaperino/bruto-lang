...
pub fn emit_runtime<'ctx>(context: &'ctx Context, module: &Module<'ctx>) {
    ...
    // bruto_write_int_fmt(val: i64, width: i64)
    {
        ...
        b.build_call(printf, &[fmt.as_pointer_value().into(), w.into(), val.into()], "").unwrap();
        b.build_return(None).unwrap();
    }
    // capture variant
    {
        ...
        b.build_call(fprintf, &[fp.into(), fmt.as_pointer_value().into(), w.into(), val.into()], "").unwrap();
        let fflush = module.get_function("fflush").unwrap();
        b.build_call(fflush, &[fp.into()], "").unwrap();
        b.build_unconditional_branch(end_bb).unwrap();
        b.position_at_end(end_bb);
    ...
    // bruto_write_real_fmt(val: f64, width: i64, prec: i64)
    {
        ...
        b.build_call(printf, &[fmt.as_pointer_value().into(), w.into(), p.into(), val.into()], "").unwrap();
        b.build_return(None).unwrap();
    }
    {
        let fn_ty = void_ty.fn_type(&[f64_ty.into(), i64_ty.into(), i64_ty.into()], false);
        ...
        let p = func.get_nth_param(2).unwrap().into_int_value();
        let fprintf = module.get_function("fprintf").unwrap();
        let fmt = b.build_global_string_ptr("%*.*f", "cap_fmt_real_wp").unwrap();
        b.build_call(fprintf, &[fp.into(), fmt.as_pointer_value().into(), w.into(), p.into(), val.into()], "").unwrap();
        let fflush = module.get_function("fflush").unwrap();
        b.build_call(fflush, &[fp.into()], "").unwrap();
        b.build_unconditional_branch(end_bb).unwrap();
        b.position_at_end(end_bb);
    ...
    }
    {
        ...
        let w = func.get_nth_param(1).unwrap().into_int_value();
        let fprintf = module.get_function("fprintf").unwrap();
        let fmt = b.build_global_string_ptr("%*.10g", "cap_fmt_real_w").unwrap();
        b.build_call(fprintf, &[fp.into(), fmt.as_pointer_value().into(), w.into(), val.into()], "").unwrap();
        let fflush = module.get_function("fflush").unwrap();
        b.build_call(fflush, &[fp.into()], "").unwrap();
        b.build_unconditional_branch(end_bb).unwrap();
    ...
    // bruto_write_str_fmt(s: ptr, width: i64)
    {
        ...
        b.build_call(printf, &[fmt.as_pointer_value().into(), w.into(), s.into()], "").unwrap();
        b.build_return(None).unwrap();
    }
    {
        let fn_ty = void_ty.fn_type(&[ptr_ty.into(), i64_ty.into()], false);
        ...
        b.build_call(fprintf, &[fp.into(), fmt.as_pointer_value().into(), w.into(), s.into()], "").unwrap();
        let fflush = module.get_function("fflush").unwrap();
        b.build_call(fflush, &[fp.into()], "").unwrap();
        b.build_unconditional_branch(end_bb).unwrap();
        b.position_at_end(end_bb);
    ...
    }
    {
        ...
        let r = b.build_call(getchar, &[], "c").unwrap()
            .try_as_basic_value().basic().unwrap().into_int_value();
    ...
    }
    {
        ...
        let buf = b.build_call(malloc, &[buf_size.into()], "buf").unwrap()
            .try_as_basic_value().basic().unwrap().into_pointer_value();
        b.build_store(buf, context.i8_type().const_int(0, false)).unwrap();
        let stdin_global = module.get_global("__stdinp").unwrap_or_else(|| {
            let g = module.add_global(ptr_ty, None, "__stdinp");
        ...
        let stdin_fp = b.build_load(ptr_ty, stdin_global.as_pointer_value(), "stdin").unwrap().into_pointer_value();
        b.build_call(fgets_fn, &[buf.into(), i32_ty.const_int(1024, false).into(), stdin_fp.into()], "").unwrap();
        let strlen = module.get_function("strlen").unwrap();
        let len = b.build_call(strlen, &[buf.into()], "L").unwrap()
            .try_as_basic_value().basic().unwrap().into_int_value();
        let zero_check = b.build_int_compare(inkwell::IntPredicate::EQ, len, i64_ty.const_int(0, false), "is0").unwrap();
        let not_zero_bb = context.append_basic_block(func, "not_zero");
        let done_bb = context.append_basic_block(func, "done");
        b.build_conditional_branch(zero_check, done_bb, not_zero_bb).unwrap();
        b.position_at_end(not_zero_bb);
        let last_idx = b.build_int_sub(len, i64_ty.const_int(1, false), "li").unwrap();
        let last_ptr = unsafe { b.build_in_bounds_gep(context.i8_type(), buf, &[last_idx], "lp").unwrap() };
        let last_ch = b.build_load(context.i8_type(), last_ptr, "lc").unwrap().into_int_value();
        let is_nl = b.build_int_compare(inkwell::IntPredicate::EQ, last_ch, context.i8_type().const_int(10, false), "isnl").unwrap();
        let strip_bb = context.append_basic_block(func, "strip");
        b.build_conditional_branch(is_nl, strip_bb, done_bb).unwrap();
        b.position_at_end(strip_bb);
        b.build_store(last_ptr, context.i8_type().const_int(0, false)).unwrap();
    ...
    // Layout: { FILE* fp; char* path; i8 buf; i8 valid; [6 bytes pad] }  = 24 bytes
    {
        ...
        let path = func.get_first_param().unwrap().into_pointer_value();
        let malloc = module.get_function("malloc").unwrap();
        let s = b.build_call(malloc, &[i64_ty.const_int(24, false).into()], "s").unwrap()
            .try_as_basic_value().basic().unwrap().into_pointer_value();
        ...
        let dup = b.build_call(strdup_fn, &[path.into()], "d").unwrap()
            .try_as_basic_value().basic().unwrap();
        let path_ptr = unsafe { b.build_in_bounds_gep(context.i8_type(), s, &[i64_ty.const_int(8, false)], "pp").unwrap() };
        b.build_store(path_ptr, dup).unwrap();
        // Initialize buffer byte and valid flag to 0
        let buf_ptr = unsafe { b.build_in_bounds_gep(context.i8_type(), s, &[i64_ty.const_int(16, false)], "buf_init").unwrap() };
        b.build_store(buf_ptr, context.i8_type().const_int(0, false)).unwrap();
        let valid_ptr = unsafe { b.build_in_bounds_gep(context.i8_type(), s, &[i64_ty.const_int(17, false)], "v_init").unwrap() };
        b.build_store(valid_ptr, context.i8_type().const_int(0, false)).unwrap();
    ...
    // bruto_file_open(f: ptr, mode: ptr) — fopens with given mode
    {
        ...
        let path_ptr = unsafe { b.build_in_bounds_gep(context.i8_type(), s, &[i64_ty.const_int(8, false)], "pp").unwrap() };
        let path = b.build_load(ptr_ty, path_ptr, "path").unwrap().into_pointer_value();
        let fopen = module.get_function("fopen").unwrap();
        let fp = b.build_call(fopen, &[path.into(), mode.into()], "fp").unwrap()
            .try_as_basic_value().basic().unwrap();
    ...
    // bruto_file_eof(f: ptr) -> i1
    {
        ...
        let end_bb = context.append_basic_block(func, "end");
        b.build_conditional_branch(is_null, null_bb, chk_bb).unwrap();
        ...
        let r = b.build_call(feof, &[fp.into()], "r").unwrap()
            .try_as_basic_value().basic().unwrap().into_int_value();
        let nz = b.build_int_compare(inkwell::IntPredicate::NE, r, i32_ty.const_int(0, false), "nz").unwrap();
        b.build_unconditional_branch(end_bb).unwrap();
        b.position_at_end(end_bb);
    ...
    // bruto_file_write_str(f: ptr, s: ptr)
    {
        ...
        b.build_call(fprintf, &[fp.into(), fmt.as_pointer_value().into(), txt.into()], "").unwrap();
        b.build_return(None).unwrap();
    }
    ...
    // bruto_file_write_int(f: ptr, val: i64)
    {
        ...
        b.build_call(fprintf, &[fp.into(), fmt.as_pointer_value().into(), v.into()], "").unwrap();
        b.build_return(None).unwrap();
    }
    ...
    // bruto_file_write_char(f: ptr, val: i8)
    {
        ...
        b.build_call(fputc_fn, &[ext.into(), fp.into()], "").unwrap();
        b.build_return(None).unwrap();
    }
    ...
    // bruto_file_writeln(f: ptr)
    {
        ...
        b.build_call(fputc_fn, &[i32_ty.const_int(10, false).into(), fp.into()], "").unwrap();
        b.build_return(None).unwrap();
    }
    ...
    // bruto_file_read_int(f: ptr) -> i64
    {
        ...
        b.build_call(fscanf, &[fp.into(), fmt.as_pointer_value().into(), tmp.into()], "").unwrap();
    ...
    // bruto_file_read_char(f: ptr) -> i8
    {
        ...
        let r = b.build_call(fgetc_fn, &[fp.into()], "r").unwrap()
            .try_as_basic_value().basic().unwrap().into_int_value();
        let trunc = b.build_int_truncate(r, i8_ty, "ct").unwrap();
        b.build_return(Some(&trunc)).unwrap();
    }
    ...
    // bruto_install_stack_guard() — calls signal(SIGSEGV)/signal(SIGBUS).
    {
        ...
        // SIGSEGV = 11
        b.build_call(signal_fn, &[i32_ty.const_int(11, false).into(), h_ptr.into()], "").unwrap();
        // SIGBUS = 10 on macOS, 7 on Linux. Install for both to be safe.
        b.build_call(signal_fn, &[i32_ty.const_int(10, false).into(), h_ptr.into()], "").unwrap();
        b.build_call(signal_fn, &[i32_ty.const_int(7, false).into(), h_ptr.into()], "").unwrap();
        b.build_return(None).unwrap();
    }
    ...
    // bruto_file_get(f: ptr) — advance buffer: f^ := next char
    {
        ...
        let c = b.build_call(fgetc_fn, &[fp.into()], "c").unwrap()
            .try_as_basic_value().basic().unwrap().into_int_value();
        let trunc = b.build_int_truncate(c, context.i8_type(), "ct").unwrap();
        let buf_ptr = unsafe { b.build_in_bounds_gep(context.i8_type(), s, &[i64_ty.const_int(16, false)], "bp").unwrap() };
        b.build_store(buf_ptr, trunc).unwrap();
        // valid := (c != -1)
        let is_eof = b.build_int_compare(inkwell::IntPredicate::EQ, c, i32_ty.const_int((-1i32) as u64, true), "is_eof").unwrap();
        ...
        let valid_ptr = unsafe { b.build_in_bounds_gep(context.i8_type(), s, &[i64_ty.const_int(17, false)], "vp").unwrap() };
    ...
    // bruto_file_put(f: ptr) — write buffer to file
    {
        ...
        let fp = b.build_load(ptr_ty, s, "fp").unwrap().into_pointer_value();
        let buf_ptr = unsafe { b.build_in_bounds_gep(context.i8_type(), s, &[i64_ty.const_int(16, false)], "bp").unwrap() };
        let buf = b.build_load(context.i8_type(), buf_ptr, "buf").unwrap().into_int_value();
        let fputc_fn = module.get_function("fputc").unwrap_or_else(|| {
            let ft = i32_ty.fn_type(&[i32_ty.into(), ptr_ty.into()], false);
            module.add_function("fputc", ft, None)
        });
    ...
    // bruto_file_buf_load(f: ptr) -> i8
    {
        ...
        b.position_at_end(bb);
        let s = func.get_first_param().unwrap().into_pointer_value();
        let buf_ptr = unsafe { b.build_in_bounds_gep(i8_ty, s, &[i64_ty.const_int(16, false)], "bp").unwrap() };
    ...
    // bruto_ioresult() -> i32  (returns and clears the error code)
    {
        ...
        b.build_store(g.as_pointer_value(), i32_ty.const_int(0, false)).unwrap();
        b.build_return(Some(&v)).unwrap();
    }
    ...
    // bruto_file_eoln(f: ptr) -> i1  (peeks next char, returns true if \n or EOF)
    {
        ...
        // EOF (-1) or '\n' counts as eoln.
        let is_eof = b.build_int_compare(inkwell::IntPredicate::EQ, c, i32_ty.const_int((-1i32) as u64, true), "is_eof").unwrap();
        let is_nl = b.build_int_compare(inkwell::IntPredicate::EQ, c, i32_ty.const_int(10, false), "is_nl").unwrap();
        ...
        b.build_conditional_branch(is_eof, chk_end_bb, push_bb).unwrap();
        ...
        phi.add_incoming(&[(&i1_ty.const_int(1, false), null_bb), (&result_chk, chk_end_bb)]);
        b.build_return(Some(&phi.as_basic_value())).unwrap();
    }
    ...
    // bruto_file_seek(f: ptr, pos: i64)
    {
        ...
        // SEEK_SET = 0
        b.build_call(fseek_fn, &[fp.into(), pos.into(), i32_ty.const_int(0, false).into()], "").unwrap();
        b.build_return(None).unwrap();
    }
    ...
    // bruto_file_filepos(f: ptr) -> i64
    {
        ...
        let r = b.build_call(ftell_fn, &[fp.into()], "r").unwrap()
            .try_as_basic_value().basic().unwrap();
        b.build_return(Some(&r)).unwrap();
    }
    ...
    // bruto_file_filesize(f: ptr) -> i64
    {
        ...
        let saved = b.build_call(ftell_fn, &[fp.into()], "saved").unwrap()
            .try_as_basic_value().basic().unwrap().into_int_value();
        // SEEK_END = 2
        b.build_call(fseek_fn, &[fp.into(), i64_ty.const_int(0, false).into(), i32_ty.const_int(2, false).into()], "").unwrap();
        let size = b.build_call(ftell_fn, &[fp.into()], "size").unwrap()
            .try_as_basic_value().basic().unwrap().into_int_value();
        b.build_call(fseek_fn, &[fp.into(), saved.into(), i32_ty.const_int(0, false).into()], "").unwrap();
        b.build_return(Some(&size)).unwrap();
    }

    // bruto_range_check_fail(line: i64, lo: i64, hi: i64, val: i64) — abort with message
    {
        let fn_ty = void_ty.fn_type(&[i64_ty.into(), i64_ty.into(), i64_ty.into(), i64_ty.into()], false);
        ...
        let stderr_g = module.get_global("__stderrp").unwrap_or_else(|| {
            let g = module.add_global(ptr_ty, None, "__stderrp");
            g.set_externally_initialized(true);
            g.set_linkage(inkwell::module::Linkage::External);
            g
        });
        let stderr_fp = b.build_load(ptr_ty, stderr_g.as_pointer_value(), "stderr").unwrap().into_pointer_value();
        let fprintf = module.get_function("fprintf").unwrap();
        let fmt = b.build_global_string_ptr(
            "Runtime error: range check failed at line %lld: %lld not in %lld..%lld\n",
            "rng_fmt",
        ).unwrap();
        b.build_call(fprintf, &[stderr_fp.into(), fmt.as_pointer_value().into(), line.into(), val.into(), lo.into(), hi.into()], "").unwrap();
        // exit(1)
    ...
    // bruto_overflow_fail(line: i64) — abort
    {
        ...
        let fprintf = module.get_function("fprintf").unwrap();
        let fmt = b.build_global_string_ptr(
            "Runtime error: integer overflow at line %lld\n", "ovf_fmt",
        ).unwrap();
        b.build_call(fprintf, &[stderr_fp.into(), fmt.as_pointer_value().into(), line.into()], "").unwrap();
        let exit_fn = module.get_function("exit").unwrap_or_else(|| {
            let ft = void_ty.fn_type(&[i32_ty.into()], false);
            module.add_function("exit", ft, None)
        });
    ...
    // bruto_file_read_str(f: ptr) -> ptr  (reads a line)
    {
        ...
        let fp = b.build_load(ptr_ty, s, "fp").unwrap().into_pointer_value();
        let malloc = module.get_function("malloc").unwrap();
        let buf = b.build_call(malloc, &[i64_ty.const_int(1024, false).into()], "buf").unwrap()
            .try_as_basic_value().basic().unwrap().into_pointer_value();
        b.build_store(buf, context.i8_type().const_int(0, false)).unwrap();
        let fgets_fn = module.get_function("fgets").unwrap_or_else(|| {
            let ft = ptr_ty.fn_type(&[ptr_ty.into(), i32_ty.into(), ptr_ty.into()], false);
        ...
        b.build_call(fgets_fn, &[buf.into(), i32_ty.const_int(1024, false).into(), fp.into()], "").unwrap();
        let strlen = module.get_function("strlen").unwrap();
        let len = b.build_call(strlen, &[buf.into()], "L").unwrap()
            .try_as_basic_value().basic().unwrap().into_int_value();
        let zero_check = b.build_int_compare(inkwell::IntPredicate::EQ, len, i64_ty.const_int(0, false), "is0").unwrap();
